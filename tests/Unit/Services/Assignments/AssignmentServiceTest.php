...
class AssignmentServiceTest extends TestCase
{
    ...
    public function test_makeAssignments_akanAkanMembuatKeSlotAssignableAvailableAgentDanMengassignTiketDenganPrioritasTiketPerView()
    {
        /** @var AssignmentRepository&\PHPUnit\Framework\MockObject\MockObject $assignmentRepoMock */
        $assignmentRepoMock = $this->createMock(AssignmentRepository::class);
        /** @var AgentRepository&\PHPUnit\Framework\MockObject\MockObject $agentRepoMock */
        $agentRepoMock = $this->createMock(AgentRepository::class);
        /** @var TicketService&\PHPUnit\Framework\MockObject\MockObject $ticketServiceMock */
        $ticketServiceMock = $this->createMock(TicketService::class);
        /** @var RoundRobinEngine&\PHPUnit\Framework\MockObject\MockObject $roundRobinEngineMock */
        $roundRobinEngineMock = $this->createMock(RoundRobinEngine::class);
        /** @var Collection&\PHPUnit\Framework\MockObject\MockObject $tasksMock */
        $tasksMock = $this->createMock(Collection::class);
        $tasksMock->method('pluck')->with($this->equalTo("zendesk_view_id"))->willReturn(collect());

        // Assert
        $agentRepoMock->expects($this->once())->method('getAvailableAgents')->with($this->anything())->willReturn($agents = factory(Agent::class, 2)->make());
        $ticketServiceMock->expects($this->once())->method('assignableByViews')->with($this->anything())->willReturn($tickets = app(TicketFactory::class)->makeMany(1));
        $roundRobinEngineMock->expects($this->once())->method('newSlot')->willReturnSelf();
        $roundRobinEngineMock->expects($this->once())->method('addAssignableAgents')->with($this->identicalTo($agents));
        $roundRobinEngineMock->expects($this->once())->method('assignWithPriority')->with($this->identicalTo($tickets));

        // Act
        $service = new AssignmentService($assignmentRepoMock, $agentRepoMock, $ticketServiceMock, $roundRobinEngineMock);
        $assignments = $service->makeAssignments("somerandomuuid", $tasksMock);       
    }
    
    /**
     * @group migratedTest
     * @deprecated moved to Unit\Zendesk\TicketServiceTest.php
     */
    public function test_makeAssignment_tidakAkanMengassignTicketYangSudahTerassign() {
        ...
        $tasksMock->method('pluck')->with($this->equalTo("zendesk_view_id"))->willReturn(collect());
        $ticketServiceMock->method('assignableByViews')->with($this->anything())->willReturn($tickets);
        $agentRepoMock->method('getAvailableAgents')->with($this->anything())->willReturn($agents);
    ...
    /**
     * @group migratedTest
     * @deprecated moved to Unit\Assignments\RoundRobinEngine method assignWithPriority
     */
